...
public class ProductListDirective extends BaseDirective {
	...
	public void execute(Environment env, Map params, TemplateModel[] loopVars, TemplateDirectiveBody body) throws TemplateException, IOException {
		...
		if ((productCategoryId != null && productCategory == null) || (brandId != null && brand == null) || (promotionId != null && promotion == null) || (tagIds != null && tags.isEmpty()) || (attributeValue != null && attributeValueMap.isEmpty())) {
			...
			List<Product> list;
			if (useCache) {
				list = productService.findList(productCategory, brand, promotion, tags, attributeValueMap, startPrice, endPrice, true, true, null, false, null, null, orderType, count, filters, orders, cacheRegion);
			} else {
				list = productService.findList(productCategory, brand, promotion, tags, attributeValueMap, startPrice, endPrice, true, true, null, false, null, null, orderType, count, filters, orders);
			}
			if(list!=null && list.size()>0){
				for (Product product : list) {
					String name=sensitivityService.replaceSensitivity(product.getName(), "**");
					product.setName(name);
					String fullName=sensitivityService.replaceSensitivity(product.getFullName(), "**");
					product.setFullName(fullName);
					String seoTitle=sensitivityService.replaceSensitivity(product.getSeoTitle(), "**");
					product.setSeoTitle(seoTitle);					
					String seoKeywords=sensitivityService.replaceSensitivity(product.getSeoKeywords(), "**");
					product.setSeoKeywords(seoKeywords);	
					String seoDescription=sensitivityService.replaceSensitivity(product.getSeoDescription(), "**");
