...
public class ReviewListDirective extends BaseDirective {
	...
	public void execute(Environment env, Map params, TemplateModel[] loopVars, TemplateDirectiveBody body) throws TemplateException, IOException {
		...
		if ((memberId != null && member == null) || (productId != null && product == null)) {
			...
			List<Review> list;
			if (useCache) {
				list = reviewService.findList(member, product, type, true, count, filters, orders, cacheRegion);
			} else {
				list = reviewService.findList(member, product, type, true, count, filters, orders);
			}
			if(list!=null && list.size()>0){
				for (Review review : list) {
					String content=review.getContent();
					content=sensitivityService.replaceSensitivity(content, "**");
