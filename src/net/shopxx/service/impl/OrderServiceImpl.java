...
public class OrderServiceImpl extends BaseServiceImpl<Order, Long> implements OrderService {
	...
	public List<Filter> getFilters() {
		...
		Filter filter1 = new Filter("merchant",Operator.eq,merchant);
		filters1.add(filter1);
		Pageable pageable1=new Pageable();
		pageable1.setFilters(filters1);
		Page<Order> orderPage=orderDao.findPage(pageable1);
		if(orderPage!=null && orderPage.getContent()!=null && orderPage.getContent().size()>0){
			Filter filter = new Filter("order",Operator.in,orderPage.getContent());
			filters.add(filter);
		}
		return filters;
	}
