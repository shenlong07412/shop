...
public class ReviewController extends BaseController {
	...
	public String list(Type type, Pageable pageable, ModelMap model,Boolean isShow) {
		model.addAttribute("type", type);
		model.addAttribute("types", Type.values());
		model.addAttribute("page", reviewService.findPage(null, null, type, isShow, pageable));
	...
	Message check( @RequestParam(defaultValue = "false") Boolean isShow,Long[] ids) {
		try {		
			reviewService.update(ids, isShow);
		} catch (Exception e) {
			e.printStackTrace();
