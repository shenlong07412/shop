...
function zTreeOnDblClick(event, treeId, treeNode) {
	$('#detail').attr("src",'${base}/admin/menu/edit.jhtml?id='+treeNode.id);
};
function addHoverDom(treeId, treeNode) {
			...
			if (btn) btn.bind("click", function(){
				$('#detail').attr("src",'${base}/admin/menu/add.jhtml?parentId='+treeNode.id);
				return false;
			});
		};
...
<body>
	<div class="path">
		<a href="${base}/admin/common/index.jhtml">${message("admin.path.index")}</a> &raquo; ${message("admin.menu.list")}
	</div>
	<div class="container">
		<div class="span6 nav">
		...
		<iframe id="detail" src="${base}/admin/menu/add.jhtml" frameborder="0" scrolling="no" width="100%" height="800px"></frame>
		</div>
	</div>
</body>
</html>